...
5. **Jira ready**: run the `jira-demo-tickets` skill to live-fetch the standing tickets from project `EC` ("Excalidraw Canvas") on `chloe-fe-demo.atlassian.net`. EC-1 (zigzag fill style), EC-2 (star fill style), and EC-3 (star shape tool) must be in "To Do". Never rely on remembered ticket text.
